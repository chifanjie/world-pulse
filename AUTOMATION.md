...
2. 执行 `python tools/safe_sync.py pull`，内部仍使用 `git pull --ff-only origin main`；成功后记录返回的远端 `head`，作为本轮发布基线。恢复行为与不可自动处理的边界见第 7 节。
...
当日内容检查后运行发布健康检查，至少覆盖最近一次成功发布后的整个日期窗口，而非只检查今天：

```powershell
python -m labs.publication_health.publication_health --since YYYY-MM-DD --through YYYY-MM-DD
```

该命令只检查本地归档，不证明已经推送。缺期、坏配对和未来生成时间不得作为幂等成功静默处理。首次恢复应明确列出缺口范围；已有历史缺口与本次新缺口分开报告。

用户要求补历史日报时，按原日期组织新闻，但 `generated_at`、`selection_as_of`、提交日期必须使用实际补发时间；历史期注明“补发”，说明原观察窗口及日期精度。只有日期、没有时区时分的来源，不能伪装成当日北京时间 09:00 前已知。不得把原日期之后的新闻写成当时已知事实，不得为了填空牺牲可验证性或凑齐 AI 数量。经过完整核验的连续缺期可作为一个明确范围的恢复批次，与索引一起原子提交；不能为贡献图逐文件机械拆分。
...
验证成功后用 `python tools/safe_sync.py push --expected-remote 本轮发布基线SHA` 普通 push 到 `origin/main`，绝不 force-push。工具检查远端未被他人推进，并在 push 后再次通过 `ls-remote` 确认远端 SHA 等于本地 HEAD；仅有本地 commit 或 push 的乐观日志不算发布成功。网络、认证、来源核验、测试或 push 失败按第 7 节分类处理，不用占位内容补图。
...
用户已于 2026-09-20 要求补发并及时修复后续错误。恢复必须限制在本仓库和本任务授权内，不能把重复失败当作“没有新内容”。

- **短暂连接错误：** `safe_sync.py` 对已结束的 DNS、连接重置与 502/503/504 错误最多尝试三次。正在执行的 Git 命令没有固定总时长上限，不因耗时较长而杀进程。
- **已知 Windows TLS 初始化错误：** 仅在同时出现 `schannel` 与 `SEC_E_NO_CREDENTIALS` 时，允许以单次命令的 `-c http.sslBackend=openssl` 重试，保持证书校验。不写全局配置、不换凭据、不关闭证书验证。若当前 Git 不支持该后端，停止并报告原错误与恢复结果。
- **认证、权限、证书错误：** 不自动绕过、不替换身份、不读取其他应用凭据；立即告知失败阶段、未发布范围及需要用户处理的事项。
- **内容和测试错误：** 对本轮生成内容或代码中原因明确的错误先最小修复，再重跑对应验证及完整测试。无法核实的事实应剔除或明确留缺，不捏造来源；若影响验收，报告尚未完成的范围。
- **并发、用户改动与未知本地提交：** 保留现场并停止，不 stash、reset、rebase、覆盖或强推。若前次 push 失败留下本自动化已验证的提交，先核对提交内容与归属、重新验证，确认远端仍是原基线后可恢复普通 push；不能因工作树干净就自动发布不明提交。
- **通知与完成依据：** 本轮一旦发生错误，无论安全恢复是否成功，都在本轮结果中简明报告错误类别、采取的修复、验证和推送结果。未恢复时明确写“未推送成功”、最近可证实的发布及当前缺口；下一次唤醒继续检查真实状态，不用 `DONT_NOTIFY` 隐藏失败。只有所有适用检查通过、没有缺口或到期未完成工作、没有发生错误且没有新成果时才 `DONT_NOTIFY`。

本地 heartbeat 依赖应用能够运行、网络和权限可用，无法承诺离线期间即时修复；恢复运行后的第一次唤醒应检查缺口并报告，不把调度记录当作执行或推送证据。
